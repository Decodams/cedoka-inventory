...
export function SalesPage() {
  ...
  useEffect(() => { setPage(1); }, [search, filterStatus, filterBranch]);
  ...
  const salesQuery = useMemo(() => {
    ...
    let q = supabase
      .from('daily_sales')
      .select(`*, product:products(id,name), branch:branches(id,name), salesperson:user_profiles!salesperson_id(full_name)`, { count: 'exact' })
    ...
    return q;
  }, [isExecutive, isBusinessLevel, user, filterBranch, filterStatus, page, pageSize]);
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
                  ...
                  return (
                    <tr key={s.id} className="hover:bg-slate-50/50">
                      ...
                      <td className="px-3 py-3">
                        <button onClick={()=>{downloadReceipt(s.id)}} className="p-1 rounded hover:bg-slate-100 text-slate-400" title="Download Receipt"><Receipt size={14}/></button>
                      </td>
                    </tr>
                  );
                })}
          ...
          <div className="p-4 border-t border-slate-100">
            <div className="flex justify-between items-center text-sm text-slate-500">
              <span>Showing {(page - 1) * pageSize + 1} to {Math.min(page * pageSize, filtered.length)} of {filtered.length} sales</span>
      ...
      {showModal && <SaleModal sale={editing} branches={branches ?? []} currentUser={user} downloadReceipt={downloadReceipt} onClose={()=>{setShowModal(false); setEditing(null);}} onSaved={()=>{refetch(); setShowModal(false); setEditing(null);}} />}
    </div>
  );
}

function SaleModal({ sale, branches, currentUser, downloadReceipt, onClose, onSaved }: { sale: DailySale | null; branches: Branch[]; currentUser: { id: string; role?: { name: string }; business_id: string | null; branch_id: string | null } | null; downloadReceipt: (saleId: string) => Promise<void>; onClose: ()=>void; onSaved: ()=>void }) {
  const isAdminOrSuper = currentUser?.role?.name === 'admin' || currentUser?.role?.name === 'super_admin';
  ...
  const handleSave = async () => {
    ...
    if (!sale && saleDate !== today) { setError(saleDate < today ? 'Sales cannot be backdated.' : 'Sale date cannot be in the future.'); return; }
    ...
    if (sale) { const r = await supabase.from('daily_sales').update(payload).eq('id', sale.id); err = r.error; }
      ...
      const r = await supabase.from('daily_sales').insert(payload).select('id').single();
      err = r.error;
      savedId = r.data?.id ?? null;
    }
    if (err) { setError(err.message); setSaving(false); return; }
    await logAudit(sale ? 'sale.updated' : 'sale.created', 'daily_sales', savedId, {
      branch_id: branchId,
      business_id: selectedBranch.business_id,
      product_id: productId ?? null,
      quantity: Number(quantity),
      total: Number(unitPrice || 0) * Number(quantity || 0) - Number(discount || 0),
    ...
    <Modal open onClose={onClose} title={sale ? 'Edit Sale' : 'Record Daily Sale'} size="lg">
      <div className="space-y-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <Select label="Branch" value={branchId} onChange={(e)=>handleBranchChange(e.target.value)} disabled={!isAdminOrSuper}>
          ...
          <Input label="Sale Date" type="date" value={saleDate} onChange={(e)=>setSaleDate(e.target.value)} min={todayIso} max={todayIso} disabled={!!sale} />
        ...
        <div className="flex justify-end gap-3 pt-2">
          ...
          <Button variant="outline" onClick={()=> downloadReceipt(sale?.id ?? '')} disabled={!sale}>
            Download Receipt
          </Button>
        </div>
      </div>
