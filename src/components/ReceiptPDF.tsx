...
export function useReceiptPDF() {
  const downloadReceipt = async (saleId: string) => {
    const { data: sale, error } = await supabase.from('daily_sales').select('*, product:products(id,name), items:sale_items(quantity,unit_price,discount_value,product:products(name)), branch:branches(name), business:businesses(name)').eq('id', saleId).single();
    if (error || !sale) throw new Error('Sale not found');
    const legacyItem = { product: sale.product, quantity: sale.quantity, unit_price: sale.unit_price, discount_value: sale.discount_value };
    ...
    pdf.autoTable({ startY: y + 3, head: [['Item', 'Qty', 'Unit price', 'Line total']], body: items.map((item: { product?: { name: string } | null; quantity: number; unit_price: number; discount_value: number }) => [item.product?.name || 'Sale item', String(item.quantity), formatCurrency(Number(item.unit_price)), formatCurrency(Number(item.quantity) * Number(item.unit_price) - Number(item.discount_value || 0))]), margin: { left: margin, right: margin }, theme: 'grid', headStyles: { fillColor: [30, 41, 59], fontSize: 8 }, bodyStyles: { fontSize: 8 } });
    y = pdf.lastAutoTable.finalY + 8;
    const paid = Number(sale.amount_paid || 0); const balance = total - paid;
    pdf.setFontSize(9); pdf.text(`Subtotal: ${formatCurrency(total + Number(sale.discount_value || 0))}`, margin, y); y += 5;
    pdf.text(`Discount: ${formatCurrency(Number(sale.discount_value || 0))}`, margin, y); y += 5;
    pdf.setFont('helvetica', 'bold'); pdf.text(`Total: ${formatCurrency(total)}`, margin, y); y += 5;
    pdf.setFont('helvetica', 'normal'); pdf.text(`Amount paid: ${formatCurrency(paid)}`, margin, y); y += 5;
